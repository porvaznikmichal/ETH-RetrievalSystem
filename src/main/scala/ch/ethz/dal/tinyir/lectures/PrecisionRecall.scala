...
object PrecisionRecall {
  ...
  // average precision
  def avgPrecision[A] (retriev: Set[A], relev: Set[A]): Double = {
    // get TP and FN
    val truePos = (retriev & relev).size
    val falseNeg = (relev -- retriev).size

    // indices in result list at which relevant items occur
    val relevIdx = retriev.zipWithIndex.filter{ case (r,_) => relev(r) }.map(_._2).toArray

    // precision values at index positions relevIdx
    val precs = relevIdx.zipWithIndex.map{case(rnk,rel)=> (rel+1)/(rnk+1).toDouble}

    // return sum devided by TP + FN
    precs.sum / min(truePos + falseNeg, 100)
  }
